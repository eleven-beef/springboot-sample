...
public class SearchController {
	...
	public ModelAndView getSalesSummary(@ModelAttribute("form") @Valid SalesForm from, BindingResult result,
			@PageableDefault(page = 0, value = 5) Pageable pageable, ModelAndView mav) {
		// MEMO:
		...
		log.trace("This log is trace log."); // (2)
		log.debug("This log is debug log."); // (3)
		log.info("This log is info log."); // (4)
		log.warn("This log is warn log."); // (5)
		...
		Page<SalesSummaryDto> salesSummaryDtoList = salesSummaryGetService.execute(salesSummaryConditionDto, pageable);

		mav.addObject("salesSummaryDtoList", salesSummaryDtoList.getContent());
		mav.addObject("page", salesSummaryDtoList);
		;
		mav.setViewName("sales");
		return mav;
	}
